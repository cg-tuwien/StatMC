...
class Tuple2 {
  public:
    ...
    Tuple2() = default;
    PBRT_CPU_GPU
    Tuple2(T x, T y) : x(x), y(y) { PBRT_V4_DCHECK(!HasNaN()); }
    PBRT_CPU_GPU
    ...
    PBRT_CPU_GPU auto operator/(U d) const -> Child<decltype(T{} / U{})> {
        PBRT_V4_DCHECK(d != 0 && !IsNaN(d));
        return {x / d, y / d};
    }
    template <typename U>
    PBRT_CPU_GPU Child<T> &operator/=(U d) {
        PBRT_V4_DCHECK_NE(d, 0);
        PBRT_V4_DCHECK(!IsNaN(d));
        x /= d;
        y /= d;
        return static_cast<Child<T> &>(*this);
    }
...
class Tuple3 {
  public:
    ...
    Tuple3() = default;
    PBRT_CPU_GPU
    Tuple3(T x, T y, T z) : x(x), y(y), z(z) { PBRT_V4_DCHECK(!HasNaN()); }
...
class Vector3 : public Tuple3<Vector3, T> {
  public:
    ...
    Vector3() = default;
    PBRT_CPU_GPU
    Vector3(T x, T y, T z) : Tuple3<pbrtv4::Vector3, T>(x, y, z) {}

    template <typename U>
    PBRT_CPU_GPU explicit Vector3(Vector3<U> v)
        : Tuple3<pbrtv4::Vector3, T>(T(v.x), T(v.y), T(v.z)) {}
...
class Point2 : public Tuple2<Point2, T> {
  public:
    ...
    Point2() { x = y = 0; }
    PBRT_CPU_GPU
    Point2(T x, T y) : Tuple2<pbrtv4::Point2, T>(x, y) {}
    template <typename U>
    PBRT_CPU_GPU explicit Point2(Point2<U> v) : Tuple2<pbrtv4::Point2, T>(T(v.x), T(v.y)) {}
    template <typename U>
    PBRT_CPU_GPU explicit Point2(Vector2<U> v)
        : Tuple2<pbrtv4::Point2, T>(T(v.x), T(v.y)) {}

    template <typename U>
    PBRT_CPU_GPU auto operator+(Vector2<U> v) const -> Point2<decltype(T{} + U{})> {
        PBRT_V4_DCHECK(!v.HasNaN());
        return {x + v.x, y + v.y};
    }
    template <typename U>
...
class Point3 : public Tuple3<Point3, T> {
  public:
    ...
    Point3() = default;
    PBRT_CPU_GPU
    Point3(T x, T y, T z) : Tuple3<pbrtv4::Point3, T>(x, y, z) {}
    ...
    PBRT_CPU_GPU explicit Point3(Point3<U> p)
        : Tuple3<pbrtv4::Point3, T>(T(p.x), T(p.y), T(p.z)) {}
    template <typename U>
    PBRT_CPU_GPU explicit Point3(Vector3<U> v)
        : Tuple3<pbrtv4::Point3, T>(T(v.x), T(v.y), T(v.z)) {}
...
class Normal3 : public Tuple3<Normal3, T> {
  public:
    ...
    Normal3() = default;
    PBRT_CPU_GPU
    Normal3(T x, T y, T z) : Tuple3<pbrtv4::Normal3, T>(x, y, z) {}
    template <typename U>
    PBRT_CPU_GPU explicit Normal3<T>(Normal3<U> v)
        : Tuple3<pbrtv4::Normal3, T>(T(v.x), T(v.y), T(v.z)) {}
...
template <typename T>
template <typename U>
Vector2<T>::Vector2(Point2<U> p) : Tuple2<pbrtv4::Vector2, T>(T(p.x), T(p.y)) {}

template <typename T>
PBRT_CPU_GPU inline auto Dot(Vector2<T> v1, Vector2<T> v2) ->
    typename TupleLength<T>::type {
    PBRT_V4_DCHECK(!v1.HasNaN() && !v2.HasNaN());
...
template <typename T>
template <typename U>
Vector3<T>::Vector3(Point3<U> p) : Tuple3<pbrtv4::Vector3, T>(T(p.x), T(p.y), T(p.z)) {}
...
PBRT_CPU_GPU inline T Dot(Vector3<T> v, Vector3<T> w) {
    PBRT_V4_DCHECK(!v.HasNaN() && !w.HasNaN());
...
template <typename T>
template <typename U>
Vector3<T>::Vector3(Normal3<U> n) : Tuple3<pbrtv4::Vector3, T>(T(n.x), T(n.y), T(n.z)) {}
...
PBRT_CPU_GPU inline auto Dot(Normal3<T> n, Vector3<T> v) ->
    typename TupleLength<T>::type {
    PBRT_V4_DCHECK(!n.HasNaN() && !v.HasNaN());
...
PBRT_CPU_GPU inline auto Dot(Vector3<T> v, Normal3<T> n) ->
    typename TupleLength<T>::type {
    PBRT_V4_DCHECK(!v.HasNaN() && !n.HasNaN());
    return FMA(n.x, v.x, SumOfProducts(n.y, v.y, n.z, v.z));
}
...
PBRT_CPU_GPU inline auto Dot(Normal3<T> n1, Normal3<T> n2) ->
    typename TupleLength<T>::type {
    PBRT_V4_DCHECK(!n1.HasNaN() && !n2.HasNaN());
...
class Bounds2 {
  public:
    ...
    Point2<T> Corner(int corner) const {
        PBRT_V4_DCHECK(corner >= 0 && corner < 4);
        return Point2<T>((*this)[(corner & 1)].x, (*this)[(corner & 2) ? 1 : 0].y);
    }
    PBRT_CPU_GPU
    Point2<T> Lerp(Point2f t) const {
        return Point2<T>(pbrtv4::Lerp(t.x, pMin.x, pMax.x),
                         pbrtv4::Lerp(t.y, pMin.y, pMax.y));
    }
    PBRT_CPU_GPU
...
class Bounds3 {
  public:
    ...
    Point3<T> Corner(int corner) const {
        PBRT_V4_DCHECK(corner >= 0 && corner < 8);
    ...
    Point3f Lerp(Point3f t) const {
        return Point3f(pbrtv4::Lerp(t.x, pMin.x, pMax.x), pbrtv4::Lerp(t.y, pMin.y, pMax.y),
                       pbrtv4::Lerp(t.z, pMin.z, pMax.z));
    }
...
PBRT_CPU_GPU inline Vector3f SphericalDirection(Float sinTheta, Float cosTheta,
                                                Float phi) {
    PBRT_V4_DCHECK(sinTheta >= -1.0001 && sinTheta <= 1.0001);
    PBRT_V4_DCHECK(cosTheta >= -1.0001 && cosTheta <= 1.0001);
...
inline Frame::Frame(Vector3f x, Vector3f y, Vector3f z) : x(x), y(y), z(z) {
    PBRT_V4_DCHECK_LT(std::abs(LengthSquared(x) - 1), 1e-4);
    PBRT_V4_DCHECK_LT(std::abs(LengthSquared(y) - 1), 1e-4);
    PBRT_V4_DCHECK_LT(std::abs(LengthSquared(z) - 1), 1e-4);
    PBRT_V4_DCHECK_LT(std::abs(Dot(x, y)), 1e-4);
    PBRT_V4_DCHECK_LT(std::abs(Dot(y, z)), 1e-4);
    PBRT_V4_DCHECK_LT(std::abs(Dot(z, x)), 1e-4);
}
