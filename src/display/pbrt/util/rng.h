...
inline int32_t RNG::Uniform<int32_t>() {
    ...
    if (v <= (uint32_t)std::numeric_limits<int32_t>::max())
        return int32_t(v);
    PBRT_V4_DCHECK_GE(v, (uint32_t)std::numeric_limits<int32_t>::min());
...
inline int64_t RNG::Uniform<int64_t>() {
    ...
    if (v <= (uint64_t)std::numeric_limits<int64_t>::max())
        ...
        return int64_t(v);
    PBRT_V4_DCHECK_GE(v, (uint64_t)std::numeric_limits<int64_t>::min());
...
inline int64_t RNG::operator-(const RNG &other) const {
    PBRT_V4_CHECK_EQ(inc, other.inc);
    ...
    while (state != curState) {
        ...
        PBRT_V4_CHECK_EQ(state & theBit, curState & theBit);
        theBit <<= 1;
