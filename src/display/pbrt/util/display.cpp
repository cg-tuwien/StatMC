...
void IPCChannel::Connect() {
    PBRT_V4_CHECK_EQ(socketFd, INVALID_SOCKET);
...
DisplayItem::ImageChannelBuffer::ImageChannelBuffer(const std::string &channelName,
                                                    int nTiles,
                                                    const std::string &title) {
    int bufferAlloc = tileSize * tileSize * sizeof(float) + channelName.size() + title.size() + 32;
...
static DisplayItem GetBufferDisplayItem(const std::string &title,
                                        const unsigned short width,
                                        const unsigned short height,
                                        const float *buffer,
                                        const std::vector<std::string> channelNames = {"R", "G", "B"}) {
    const unsigned char nChannels = channelNames.size();

    auto getValues = [=](Bounds2i b, pstd::span<pstd::span<float>> displayValues) {
        unsigned int offset = 0;
        for (Point2i p : b) {
            for (unsigned char i = 0; i < nChannels; ++i)
                displayValues[i][offset] = buffer[(p.y * width + p.x) * nChannels + i];
            ++offset;
        }
    };

    return DisplayItem(title, Point2i(width, height), channelNames, getValues);
}
...
void DisplayStatic(const std::string title,
                   const unsigned short width,
                   const unsigned short height,
                   const float *buffer,
                   const std::vector<std::string> channelNames) {
    DisplayItem item = GetBufferDisplayItem(title, width, height, buffer, channelNames);
...
void DisplayDynamic(const std::string title,
                    const unsigned short width,
                    const unsigned short height,
                    const float *buffer,
                    const std::vector<std::string> channelNames) {
    std::lock_guard<std::mutex> lock(mutex);
    dynamicItems.push_back(GetBufferDisplayItem(title, width, height, buffer, channelNames));
}
