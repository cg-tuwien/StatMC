...
pstd::array<Float, 3> SampleSphericalTriangle(const pstd::array<Point3f, 3> &v, Point3f p,
    ...
    PBRT_V4_CHECK_GT(LengthSquared(a), 0);
    PBRT_V4_CHECK_GT(LengthSquared(b), 0);
    PBRT_V4_CHECK_GT(LengthSquared(c), 0);
    a = Normalize(a);
    b = Normalize(b);
    c = Normalize(c);
    ...
    PBRT_V4_CHECK_RARE(1e-6, divisor == 0);
    if (divisor == 0) {
...
Point2f InvertSphericalTriangleSample(const pstd::array<Point3f, 3> &v, Point3f p,
    ...
    if (Dot(a, cp) > 0.99999847691f /* 0.1 degrees */)
        ...
        PBRT_V4_CHECK_RARE(1e-5, LengthSquared(n_cpb) == 0 || LengthSquared(n_acp) == 0);
...
Point3f SampleSphericalRectangle(Point3f pRef, Point3f s, Vector3f ex, Vector3f ey,
    ...
    PBRT_V4_CHECK_RARE(1e-5, solidAngle <= 0);
    if (solidAngle <= 0) {
        if (pdf)
            *pdf = 0;
...
Float SampleCatmullRom(pstd::span<const Float> nodes, pstd::span<const Float> f,
    ...
    PBRT_V4_CHECK_EQ(nodes.size(), f.size());
    PBRT_V4_CHECK_EQ(f.size(), F.size());
...
int AliasTable::Sample(Float u, Float *pmf, Float *uRemapped) const {
    ...
    if (up < bins[offset].q) {
        ...
        PBRT_V4_DCHECK_GT(bins[offset].p, 0);
        if (pmf)
            *pmf = bins[offset].p;
        if (uRemapped)
        ...
        PBRT_V4_DCHECK_GE(alias, 0);
        PBRT_V4_DCHECK_GT(bins[alias].p, 0);
        if (pmf)
            *pmf = bins[alias].p;
        if (uRemapped)
