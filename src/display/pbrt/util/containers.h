...
class Array2D {
  public:
    ...
    T &operator[](Point2i p) {
        PBRT_V4_DCHECK(InsideExclusive(p, extent));
        p.x -= extent.pMin.x;
        p.y -= extent.pMin.y;
...
class HashMap {
  public:
    ...
    const Value &operator[](const Key &key) const {
        size_t offset = FindOffset(key);
        PBRT_V4_CHECK(table[offset].has_value());
...
class SampledGrid {
  public:
    ...
    SampledGrid(pstd::span<const T> v, int nx, int ny, int nz, Allocator alloc)
        ...
        PBRT_V4_CHECK_EQ(nx * ny * nz, values.size());
    }
