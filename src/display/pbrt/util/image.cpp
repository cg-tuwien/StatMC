...
// // too old school
// #define STBI_NO_PIC
// #define STBI_ASSERT PBRT_V4_CHECK
...
void ForExtent(const Bounds2i &extent, WrapMode2D wrapMode, const Image &image, F op) {
    PBRT_V4_CHECK_LT(extent.pMin.x, extent.pMax.x);
    PBRT_V4_CHECK_LT(extent.pMin.y, extent.pMax.y);
    ...
    if (Intersect(extent, Bounds2i({0, 0}, image.Resolution())) == extent) {
        ...
        for (int y = extent.pMin[1]; y < extent.pMax[1]; ++y) {
            for (int x = 0; x < nx; ++x) {
                ...
                PBRT_V4_CHECK(RemapPixelCoords(&p, image.Resolution(), wrapMode));
...
std::vector<ResampleWeight> Image::ResampleWeights(int oldRes, int newRes) {
    PBRT_V4_CHECK_GE(newRes, oldRes);
...
Image Image::FloatResizeUp(Point2i newRes, WrapMode2D wrapMode) const {
    PBRT_V4_CHECK_GE(newRes.x, resolution.x);
    PBRT_V4_CHECK_GE(newRes.y, resolution.y);
    ...
    ParallelFor2D(Bounds2i({0, 0}, newRes), [&](Bounds2i outExtent) {
        ...
        for (int yOut = inExtent.pMin.y; yOut < inExtent.pMax.y; ++yOut) {
            for (int xOut = outExtent.pMin.x; xOut < outExtent.pMax.x; ++xOut) {
                ...
                PBRT_V4_DCHECK(xOut >= 0 && xOut < xWeights.size());
                ...
                PBRT_V4_DCHECK_GE(xIn, 0);
                PBRT_V4_DCHECK_LT(xIn + 3, nxIn);
                int yIn = yOut - inExtent.pMin.y;
                int inOffset = NChannels() * (xIn + yIn * nxIn);
                PBRT_V4_DCHECK_GE(inOffset, 0);
                PBRT_V4_DCHECK_LT(inOffset + 3 * NChannels(), inBuf.size());
        ...
        for (int x = 0; x < nxOut; ++x) {
            for (int y = 0; y < nyOut; ++y) {
                ...
                PBRT_V4_DCHECK(yOut >= 0 && yOut < yWeights.size());
                const ResampleWeight &rsw = yWeights[yOut];

                PBRT_V4_DCHECK_GE(rsw.firstPixel - inExtent[0][1], 0);
                int xBufOffset =
                    NChannels() * (x + nxOut * (rsw.firstPixel - inExtent[0][1]));
                PBRT_V4_DCHECK_GE(xBufOffset, 0);
                int step = NChannels() * nxOut;
                PBRT_V4_DCHECK_LT(xBufOffset + 3 * step, xBuf.size());
...
pstd::vector<Image> Image::GeneratePyramid(Image image, WrapMode2D wrapMode,
    ...
    PBRT_V4_CHECK(Is32Bit(image.format));
    ...
    PBRT_V4_CHECK(image.resolution[0] == 1 && image.resolution[1] == 1);
...
void Image::SetChannels(Point2i p, const ImageChannelDesc &desc,
    ...
    PBRT_V4_CHECK_LE(values.size(), NChannels());
...
Image::Image(pstd::vector<uint8_t> p8c, Point2i resolution,
    ...
    : format(PixelFormat::U256),
      ...
      encoding(encoding),
      p8(std::move(p8c)) {
    PBRT_V4_CHECK_EQ(p8.size(), NChannels() * resolution[0] * resolution[1]);
}
...
Image::Image(pstd::vector<Half> p16c, Point2i resolution,
    ...
    : format(PixelFormat::Half),
      ...
      channelNames(channels.begin(), channels.end()),
      p16(std::move(p16c)) {
    PBRT_V4_CHECK_EQ(p16.size(), NChannels() * resolution[0] * resolution[1]);
    PBRT_V4_CHECK(Is16Bit(format));
}
...
Image::Image(pstd::vector<float> p32c, Point2i resolution,
    ...
    : format(PixelFormat::Float),
      ...
      channelNames(channels.begin(), channels.end()),
      p32(std::move(p32c)) {
    PBRT_V4_CHECK_EQ(p32.size(), NChannels() * resolution[0] * resolution[1]);
    PBRT_V4_CHECK(Is32Bit(format));
}
...
ImageChannelValues Image::MAE(const ImageChannelDesc &desc, const Image &ref,
    ...
    PBRT_V4_CHECK((bool)refDesc);
    PBRT_V4_CHECK_EQ(Resolution(), ref.Resolution());
...
void Image::CopyRectOut(const Bounds2i &extent, pstd::span<float> buf,
                        WrapMode2D wrapMode) const {
    PBRT_V4_CHECK_GE(buf.size(), extent.Area() * NChannels());
...
Image Image::JointBilateralFilter(const ImageChannelDesc &toFilterDesc, int halfWidth,
    ...
    PBRT_V4_CHECK_EQ(jointDesc.size(), jointSigma.size());
...
bool Image::Write(std::string name, const ImageMetadata &metadata) const {
    if (metadata.pixelBounds)
        PBRT_V4_CHECK_EQ(metadata.pixelBounds->Area(), resolution.x * resolution.y);
    ...
    PBRT_V4_CHECK(outImage.NChannels() == 1 || outImage.NChannels() == 3 ||
...
static ImageAndMetadata ReadEXR(const std::string &name, Allocator alloc) {
    try {
        ...
        PBRT_V4_CHECK(pixelType == Imf::HALF || pixelType == Imf::FLOAT);
...
bool Image::WriteEXR(const std::string &name, const ImageMetadata &metadata) const {
    ...
    PBRT_V4_CHECK(Is16Bit(format) || Is32Bit(format));
...
//                 }
//             PBRT_V4_DCHECK(bufIter == buf.end());
//         } else {
...
Image Image::Crop(const Bounds2i &bounds, Allocator alloc) const {
    PBRT_V4_CHECK_GT(bounds.Area(), 0);
    PBRT_V4_CHECK(bounds.pMin.x >= 0 && bounds.pMin.y >= 0);
...
//     PBRT_V4_CHECK(pixels != nullptr);  // qoi failure
...
//     else
//         PBRT_V4_CHECK_EQ(3, desc.channels);

//     PBRT_V4_CHECK(desc.colorspace == QOI_SRGB || desc.colorspace == QOI_LINEAR);
//     ColorEncoding encoding =
