...
bool HasExtension(std::string filename, std::string e) {
    ...
    std::string filenameExtension = filesystem::path(filename).extension().string();
...
std::string ResolveFilename(std::string filename) {
    ...
    if (filesystem::exists(filepath))
        return filesystem::absolute(filepath);
    return filename;
}
...
std::vector<std::string> MatchingFilenames(std::string filenameBase) {
    ...
    if (!dir)
        ErrorExit("%s: unable to open directory\n", basePath.parent_path().string().c_str());

    struct dirent *ent;
    size_t n = basePath.filename().string().size();
    while ((ent = readdir(dir)) != nullptr) {
        if (ent->d_type == DT_REG &&
            strncmp(basePath.filename().c_str(), ent->d_name, n) == 0)
            filenames.push_back(
                (basePath.parent_path() / filesystem::path(ent->d_name)).string());
    }
    closedir(dir);
#endif
...
//     PBRT_V4_CHECK_GT(compressed.size(), 4);
...
//             PBRT_V4_CHECK_EQ(actualOut, decompressed.size());
    ...
    while ((c = getc(f)) != EOF) {
        ...
        if (inNumber) {
            ...
            if ((isdigit(c) != 0) || c == '.' || c == 'e' || c == 'E' || c == '-' ||
                c == '+') {
                PBRT_V4_CHECK_LT(curNumberPos, sizeof(curNumber));
