...
class SampledSpectrum {
  public:
    ...
    SampledSpectrum &operator/=(const SampledSpectrum &s) {
        for (int i = 0; i < NSpectrumSamples; ++i) {
            PBRT_V4_DCHECK_NE(0, s.values[i]);
            values[i] /= s.values[i];
        }
        return *this;
    }
    ...
    SampledSpectrum(pstd::span<const Float> v) {
        PBRT_V4_DCHECK_EQ(NSpectrumSamples, v.size());
        for (int i = 0; i < NSpectrumSamples; ++i)
            values[i] = v[i];
    }
    ...
    Float operator[](int i) const {
        PBRT_V4_DCHECK(i >= 0 && i < NSpectrumSamples);
        return values[i];
    }
    PBRT_CPU_GPU
...
class DenselySampledSpectrum {
  public:
    ...
    DenselySampledSpectrum(Spectrum spec, int lambda_min = Lambda_min,
        ...
        : lambda_min(lambda_min),
          lambda_max(lambda_max),
          values(lambda_max - lambda_min + 1, alloc) {
        PBRT_V4_CHECK_GE(lambda_max, lambda_min);
        if (spec)
  ...
  private:
    friend struct std::hash<pbrtv4::DenselySampledSpectrum>;
...
PBRT_CPU_GPU inline SampledSpectrum Clamp(const SampledSpectrum &s, U low, V high) {
    SampledSpectrum ret;
    for (int i = 0; i < NSpectrumSamples; ++i)
        ret[i] = pbrtv4::Clamp(s[i], low, high);
    PBRT_V4_DCHECK(!ret.HasNaNs());
    return ret;
}
...
template <>
struct hash<pbrtv4::DenselySampledSpectrum> {
    PBRT_CPU_GPU
    size_t operator()(const pbrtv4::DenselySampledSpectrum &s) const {
        return pbrtv4::HashBuffer(s.values.data(), s.values.size());
    }
};
