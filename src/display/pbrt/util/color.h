...
PBRT_CPU_GPU inline RGB Clamp(RGB rgb, U min, V max) {
    return RGB(pbrtv4::Clamp(rgb.r, min, max), pbrtv4::Clamp(rgb.g, min, max),
               pbrtv4::Clamp(rgb.b, min, max));
}
...
PBRT_CPU_GPU inline XYZ Clamp(const XYZ &xyz, U min, V max) {
    return XYZ(pbrtv4::Clamp(xyz.X, min, max), pbrtv4::Clamp(xyz.Y, min, max),
               pbrtv4::Clamp(xyz.Z, min, max));
}
...
class LinearColorEncoding {
  public:
    ...
    void ToLinear(pstd::span<const uint8_t> vin, pstd::span<Float> vout) const {
        PBRT_V4_DCHECK_EQ(vin.size(), vout.size());
