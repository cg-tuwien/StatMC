...
uint32_t GetThreadIndex() {
    ...
    uint64_t tid;
    PBRT_V4_CHECK_EQ(pthread_threadid_np(pthread_self(), &tid), 0);
    return tid;
#else
...
#error "Need to implement sleepms() for current platform"
        ...
        auto getCPUUsage = [&]() -> Usage {
            ...
            while (std::getline(stat, line)) {
                ...
                PBRT_V4_CHECK_GE(values.size(), 4);
                usage.user = values[0];
                usage.nice = values[1];
            ...
            PBRT_V4_CHECK((bool)io);
            while (std::getline(io, line)) {
                if (line.compare(0, 7, "rchar: ") == 0)
                    PBRT_V4_CHECK(Atoi(line.data() + 7, &usage.readRequest));
                if (line.compare(0, 7, "wchar: ") == 0)
                    PBRT_V4_CHECK(Atoi(line.data() + 7, &usage.writeRequest));
                if (line.compare(0, 12, "read_bytes: ") == 0)
                    PBRT_V4_CHECK(Atoi(line.data() + 12, &usage.readActual));
                if (line.compare(0, 13, "write_bytes: ") == 0)
                    PBRT_V4_CHECK(Atoi(line.data() + 13, &usage.writeActual));
            }
#elif defined(PBRT_IS_OSX)
            // possibly useful:
